...
def get_conversations():
    conversation_keys = [key for key in db.keys() if key.startswith("conversation-") and key.endswith("-history")]
    conversation_ids = [key.replace("conversation-", "").replace("-history", "") for key in conversation_keys]
...
def get_conversation_history():
    ...
    return jsonify({"success": True, "conversation_history": dict(conversation_history)})
